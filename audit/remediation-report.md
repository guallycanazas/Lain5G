...
| Source licensing | `COMPLETED` for the stated scope | `LICENSE` and byte-identical `Licence.txt` cover first-party MIT source; imported BSD configuration has exact provenance and notice. | This conclusion does not cover third-party binaries as one aggregate work. |
